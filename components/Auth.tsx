...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleDemoLogin = () => {
    ...
    // Check if demo user already exists in mock storage
    const users = storageService.getUsers();
    ...
    // Slight delay for aesthetic effect before logging in
    setTimeout(() => {
      onLogin(demoUser);
    }, 500);
  };
    ...
    <div className="max-w-md mx-auto mt-20 px-4">
      <div className="bg-white p-8 rounded-2xl shadow-2xl border border-slate-200 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <button 
              type="submit"
              className="w-full py-4 bg-indigo-600 text-white font-black text-lg rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 hover:shadow-indigo-200 transform active:scale-[0.98]"
            >
            ...
            <button 
              ...
              className="w-full py-4 bg-indigo-50 text-indigo-600 font-black text-lg rounded-xl border-2 border-dashed border-indigo-200 hover:bg-indigo-100 hover:border-indigo-300 transition-all transform active:scale-[0.98] flex items-center justify-center gap-3"
            >
      ...
      <div className="mt-8 text-center px-6">
        <p className="text-xs text-slate-400 font-medium">
          Note: Demo account uses <strong>demo@resumepro.com</strong> and password <strong>demo1234</strong>. 
